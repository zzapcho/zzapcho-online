...
- `release-launcher.yml`: 태그 또는 수동 실행으로 Windows 설치 파일을 GitHub Release에 업로드

`main`에 모드/리소스팩 변경이 push되면 `verify-content.yml`이 manifest를 다시 만들고 변경분이 있으면 자동 커밋합니다. 로컬에서는 배포 전에 `npm run manifest`와 `npm run content:verify`를 실행해 확인하는 것을 권장합니다.
...
런처 실행 중 새 버전이 감지되거나 업데이트 다운로드가 준비되면 사이드바의 `업데이트` 항목에 골드 테두리가 표시됩니다. 다운로드가 끝나면 같은 위치에서 재시작 버튼으로 업데이트를 적용합니다.
...
`vX.Y.Z` 태그가 push되면 GitHub Actions가 Windows 설치 파일 `ZzapchoOnline-Setup-X.Y.Z.exe`, `latest.yml`, blockmap을 GitHub Release에 업로드합니다. Actions 탭에서 `Release Launcher`를 수동 실행해도 되며, 태그 입력을 비우면 `package.json`의 `version`으로 `vX.Y.Z` 태그명을 사용합니다.
